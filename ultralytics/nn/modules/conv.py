...
class UpSampleGN(nn.Module):
    ...
    def __init__(self, dim, group, size, scale_factor, mode):  # ch_in, ch_out, kernel, stride, dilation, activation
        super().__init__()
        self.upsample = nn.Upsample(size=size, scale_factor=scale_factor, mode=mode)
        self.gn = nn.GroupNorm(num_groups=group, num_channels=dim)

    def forward(self, x):
        return self.gn(self.upsample(x))
