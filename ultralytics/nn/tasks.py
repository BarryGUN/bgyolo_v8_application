...
from ultralytics.nn.modules.block import MS2, MS2b, C2RepX, SplitMP, SPPFCSP, MS2d, MS2e, C2d
        ...
        if not self.is_fused():
            for m in self.model.modules():
                ...
                if isinstance(m, RepConv):
                    m.fuse_convs()
                    m.forward = m.forward_fuse  # update forward
                if isinstance(m, (RepXConv, RepXConvCCAB, RepXConvCBCBA)):
                    m.switch_to_deploy()
                    m.forward = m.forward_fuse  # update forward
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in (Classify, Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, SPPFCSP, DWConv, Focus,
                 ...
                 MS2, MS2b, C2RepX, SplitMP, MS2d, MS2e, C2RepXCCAB, C2RepXCBCBA, C2d):
            c1, c2 = ch[f], args[0]
            ...
            if m in (BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, C3x, RepC3, MS2, MS2b, C2RepX,
                     MS2d, MS2e, C2RepXCCAB, C2RepXCBCBA,C2d):
